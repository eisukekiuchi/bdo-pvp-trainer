...
const key = Object.create(null); let yaw=0, pitch=-0.18; let started=false; let last=performance.now();
...
function makeActor(team,name,pos,isPlayer=false){
  ...
  const bodyMat=mat(team==='blue'?0x355f9b:0x8c3f40); const dark=mat(0x20252a);
  const body=new THREE.Mesh(new THREE.CapsuleGeometry(.45,1.05,5,10),bodyMat);body.position.y=1.3;body.castShadow=true;group.add(body);
  ...
  const ring=new THREE.Mesh(new THREE.RingGeometry(.62,.75,32),new THREE.MeshBasicMaterial({color:team==='blue'?0x50a4ff:0xff6464,side:THREE.DoubleSide,transparent:true,opacity:.75}));ring.rotation.x=-Math.PI/2;ring.position.y=.035;group.add(ring);
  return {group,team,name,isPlayer,hp:100,maxHp:100,speed:isPlayer?8:4.5,cc:0,invuln:0,sa:0,fg:false,attackCd:0,decision:Math.random()*.5,target:null,alive:true,body,dodgeCd:0};
}
...
function setProtectionVisual(a){let c=a.team==='blue'?0x355f9b:0x8c3f40;if(a.cc>0)c=0xe58a32;else if(a.invuln>0)c=0x8a5ed1;else if(a.sa>0)c=0x397cc7;else if(a.fg)c=0x3e9b63;a.body.material.color.setHex(c);}
...
function playerUpdate(dt){if(!player||!player.alive)return;player.fg=!!key.KeyQ;
  const fwd=new THREE.Vector3(Math.sin(yaw),0,Math.cos(yaw));const right=new THREE.Vector3(fwd.z,0,-fwd.x);const mv=new THREE.Vector3();
  if(key.KeyW)mv.add(fwd);if(key.KeyS)mv.sub(fwd);if(key.KeyD)mv.add(right);if(key.KeyA)mv.sub(right);if(mv.lengthSq()>0){mv.normalize();moveActor(player,mv,dt);face(player,player.group.position.clone().add(mv),16,dt);} }

function actPlayer(type){if(!player||!player.alive||player.attackCd>0)return;let target=nearestEnemy(player);if(type==='light'){player.attackCd=.32;deal(player,target,'light');}
  if(type==='sa'){player.attackCd=.75;player.sa=.55;deal(player,target,'sa');ui.event.textContent='Super Armor strike';}
  if(type==='cc'){player.attackCd=.62;deal(player,target,'cc');}
  if(type==='grab'){player.attackCd=.9;deal(player,target,'grab');}
  if(type==='iframe'&&player.dodgeCd<=0){player.invuln=.38;player.dodgeCd=.9;const f=new THREE.Vector3(Math.sin(yaw),0,Math.cos(yaw));player.group.position.addScaledVector(f,2.1);ui.event.textContent='iframe dodge';}}
...
function timers(a,dt){a.cc=Math.max(0,a.cc-dt);a.invuln=Math.max(0,a.invuln-dt);a.sa=Math.max(0,a.sa-dt);a.attackCd=Math.max(0,a.attackCd-dt);a.dodgeCd=Math.max(0,a.dodgeCd-dt);setProtectionVisual(a);}
...
function uiUpdate(){if(!player)return;const hp=clamp(player.hp/player.maxHp,0,1);ui.hpFill.style.width=`${hp*100}%`;ui.hpText.textContent=`HP ${Math.ceil(player.hp)} / ${player.maxHp}`;ui.protection.textContent=player.cc>0?'CC':player.invuln>0?'IFRAME':player.sa>0?'SA':player.fg?'FG':'NONE';ui.metrics.textContent=`Hits ${metrics.hits}/${metrics.attempts} ・ CC ${metrics.cc} ・ Grab ${metrics.grabs} ・ Deaths ${metrics.deaths}`;}
...
window.addEventListener('keydown',e=>{key[e.code]=true;if(e.repeat)return;if(e.code==='KeyE')actPlayer('sa');if(e.code==='KeyF')actPlayer('cc');if(e.code==='KeyR')actPlayer('grab');if(e.code==='Space'){e.preventDefault();actPlayer('iframe');}});
window.addEventListener('keyup',e=>{key[e.code]=false;});
canvas.addEventListener('mousedown',e=>{if(!started)return;if(document.pointerLockElement!==canvas){canvas.requestPointerLock?.();return;}if(e.button===0)actPlayer('light');});
